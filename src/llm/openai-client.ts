...
import type { GenerateWithToolsInput, GenerateWithToolsResult, LlmMessage, StreamWithToolsInput, StreamWithToolsResult, ToolCall } from './types.js'
...
export class OpenAiClient implements LlmClient {
  ...
  static async create(model?: string): Promise<OpenAiClient> {
    ...
    const apiKey = envKey && envKey.trim().length > 0 ? envKey.trim() : await OpenAiClient.loadFromKeyStore()
  ...
  async generateWithTools(input: GenerateWithToolsInput): Promise<GenerateWithToolsResult> {
    ...
    for (let step = 0; step < maxSteps; step++) {
      ...
      const assistantMsg: LlmMessage = { role: 'assistant', content: assistantText, toolCalls: assistantToolCalls }
      newMessages.push(assistantMsg)
      history = history.concat([assistantMsg])

      for (const tc of toolCalls) {
        const parsedArgs = OpenAiClient.safeJsonParse(tc.function?.arguments) ?? {}
        const call: ToolCall = { id: tc.id, name: tc.function?.name, arguments: parsedArgs }
  ...
  async streamWithTools(input: StreamWithToolsInput): Promise<StreamWithToolsResult> {
    const maxSteps = input.maxSteps ?? 8
    const newMessages: LlmMessage[] = []
    ...
    for (let step = 0; step < maxSteps; step++) {
      ...
      if (streamed.toolCalls.length === 0) {
        if (streamed.assistantText.trim().length > 0) {
          const m: LlmMessage = { role: 'assistant', content: streamed.assistantText }
          newMessages.push(m)
          history = history.concat([m])
        }
      ...
      const assistantMsg: LlmMessage = {
        ...
        toolCalls: streamed.toolCalls.map(tc => ({ id: tc.id, name: tc.name, arguments: tc.arguments }))
      }
      newMessages.push(assistantMsg)
      history = history.concat([assistantMsg])
  ...
  private async callChatCompletions(input: { messages: OpenAiChatMessage[]; tools: any[] }): Promise<any> {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a coding assistant. Use tools when needed. When you need a tool, call it via tool_calls. Keep responses concise.'
          },
          ...input.messages
        ],
    ...
    if (!res.ok) {
      ...
      throw new Error(`OpenAI request failed: ${res.status} ${res.statusText}${txt ? `: ${txt}` : ''}`)
    }
    return res.json()
  }
  ...
  private toOpenAiMessages(messages: LlmMessage[]): OpenAiChatMessage[] {
    return messages.map(m => {
      if (m.role === 'tool') {
        return { role: 'tool', content: m.content, tool_call_id: m.toolCallId ?? 'tool_call' }
      }
  ...
  }): Promise<{ assistantText: string; toolCalls: { id: string; name: string; arguments: string }[] }> {
    const res = await fetch(`${this.baseUrl}/chat/completions`, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
    ...
    while (true) {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (data === '[DONE]') {
            return { assistantText, toolCalls: Array.from(toolCallByIndex.entries()).sort((a, b) => a[0] - b[0]).map(([, v]) => v) }
          }
          ...
          if (Array.isArray(toolDeltas)) {
            for (const td of toolDeltas) {
              ...
              const current = toolCallByIndex.get(idx) ?? { id: '', name: '', arguments: '' }
