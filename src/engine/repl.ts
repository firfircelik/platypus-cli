...
export function createRepl(prompt: string, handlers: ReplHandlers): Repl {
  ...
  rl.setPrompt(prompt)
  ;(rl as any).historySize = 500

  const historyPath = path.join(getPlatypusHome(), 'history')
  const initialHistory = readHistory(historyPath)
  if (initialHistory.length > 0) (rl as any).history = initialHistory.slice(0, 500).reverse()
...
function writeHistory(filePath: string, initial: string[], appended: string[]): void {
  try {
