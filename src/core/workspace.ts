...
export class Workspace {
  ...
  async writeFile(agentId: string, relPath: string, content: string): Promise<void> {
    const full = this.resolve(relPath)
    let lock
    try {
      lock = await this.locks.acquireLock(agentId, full)
    } catch (error) {
      const msg = error instanceof Error ? error.message : 'Lock acquisition failed'
    ...
    try {
      ...
      this.audit.write({
        ...
        action: 'file.write',
        resource: full,
        details: { sha256: crypto.createHash('sha256').update(content).digest('hex') }
      })
    } finally {
