...
vi.mock('../src/state/agent-task-store.js', () => ({
  AgentTaskStore: class {
    ...
    get() {
      const status = getCalls++ === 0 ? 'pending' : 'done'
...
describe('CLI run wait', () => {
  it('waits and prints results', async () => {
    getCalls = 0
    vi.useFakeTimers()
    const cmd = new Run([], {} as any)
    ;(cmd as any).parse = vi.fn(async () => ({ args: { task: 'do' }, flags: { ensureTeam: false, wait: 1 } }))
