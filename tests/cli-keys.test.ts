...
vi.mock("../src/crypto/providers.js", () => ({
  validateProviderKey: vi.fn(async (provider: string) => ({
    valid: true,
    provider,
  })),
}));
...
describe("CLI keys commands", () => {
  it("keys add stores key", async () => {
    const store = {
      initialize: vi.fn(),
      storeKey: vi.fn(async () => undefined),
    };
    ...
    await cmd.run();
    expect(store.storeKey).toHaveBeenCalledWith(
      "openai",
  ...
  it("keys add prompts when key omitted", async () => {
    ...
    (cmd as any).parse = vi.fn(async () => ({
      args: {},
      flags: { provider: "openai", key: undefined, validate: false },
    }));
    (cmd as any).getKeyStore = vi.fn(async () => store);
    (cmd as any).log = vi.fn();
    await cmd.run();
  ...
  it("keys validate errors when invalid", async () => {
    (validateProviderKey as any).mockImplementationOnce(async () => ({
      valid: false,
      provider: "openai",
      error: "bad",
    }));
